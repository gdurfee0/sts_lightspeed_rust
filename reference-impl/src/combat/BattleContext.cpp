...
void BattleContext::initRelics(const GameContext &gc)
{
    ...
    for (const auto &r : gc.relics.relics)
    {
        ...
        case R::SLAVERS_COLLAR:
            ...
            { // todo this needs to be set by eliteTrigger maybe?
                p.energyPerTurn++;
            }
            break;
...
void BattleContext::exitBattle(GameContext &g) const
{
    ...
    BattleContext::sum += g.curHp + g.maxHp + g.gold + g.act + g.ascension + g.floorNum + potionRng.counter + cardRandomRng.counter;
}
...
void BattleContext::playCardQueueItem(CardQueueItem playItem)
{
    ...
    { // not sure if this is correct,
        ...
        if (!c.requiresTarget() || monsters.arr[item.target].isTargetable())
        { // this is redundant right???? -> no i think echo form abilities can queue a card with invalid target
            useCard();
        }
    }
...
void BattleContext::useCard()
{
    ...
    if (!item.purgeOnUse)
    { // todo change to checking the card queue item
...
void BattleContext::useAttackCard()
{
    ...
    case CardId::RAMPAGE:
    { // todo doubled card damage is changed by the first
    ...
    { // todo test with breaking block on first hit and drill relic
...
void BattleContext::onAfterUseCard()
{
    ...
    else
    {
        ...
        if (rebound)
        { // these methods all call resetCardBeforeMoving but it looks unnecessary
...
void BattleContext::drawCards(int count)
{
    ...
    for (int i = 0; i < cards.drawPile.size(); ++i)
    {
        std::cout << "  drawPile " << i << " is " << cards.drawPile[i] << std::endl;
    }
...
void BattleContext::chooseExhaustCards(const fixed_list<int, 10> &idxs)
{
    ...
    auto listCopy = idxs;
    std::sort(listCopy.begin(), listCopy.end(), [](auto a, auto b)
              { return b < a; });
